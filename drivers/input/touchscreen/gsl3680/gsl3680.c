...
static void gsl_load_fw(struct i2c_client *client)
{
	...
	ptr_fw = (struct fw_data *)GSL3680_FW_AP106;
	source_len = ARRAY_SIZE(GSL3680_FW_AP106);
#else
...
static int gslX680_ts_init(struct i2c_client *client, struct gsl_ts *ts)
{
	...
	input_mt_init_slots(input_device, MAX_CONTACTS);
			...
			const struct i2c_device_id *id)
{
	...
	gpio_reset = pdata->reset_gpio;
	ts->irq = gpio_to_irq(pdata->touch_en_gpio);
