...
static void gsl_load_fw(struct i2c_client *client)
{
	...
	printk("read 0xfc = 0x%02x 0x%02x 0x%02x 0x%02x\n", read_buf[3], read_buf[2], read_buf[1], read_buf[0]);
...
static void process_gslX680_data(struct gsl_ts *ts)
{
	...
	{
		x = join_bytes((ts->touch_data[ts->dd->x_index  + 4 * i + 1] & 0xff),
				ts->touch_data[ts->dd->x_index + 4 * i]);
		y = join_bytes(ts->touch_data[ts->dd->y_index + 4 * i + 1],
				ts->touch_data[ts->dd->y_index + 4 * i ]);
		id = ts->touch_data[ts->dd->id_index + 4 * i] >> 4;

		print_info("gslx680 i, id: %d, %d\n", i, id);
		print_info("gslx680 i, x, y: %d, %d, %d\n", i, x, y);

		if (0 <= id && id <= MAX_CONTACTS)  // <= 1
		{
		#ifdef FILTER_POINT
			filter_point(x, y , id);
		#else
			...
			const struct i2c_device_id *id)
{
	...
	printk("gslx680 req reset = 0x%08x\n", ts_global_reset_pin);
